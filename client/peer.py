...
class Peer(object):
    ...
    def connect(self):
        try:
            ...
            print(
                "Failed to connect to peer (ip: %s - port: %s - %s)"
                % (self.ip, self.port, e.__str__())
            )
            return False
    ...
    def handle_request(self, request):
        ...
        if self.is_interested() and self.is_unchoked():
            pub.sendMessage(
                "PiecesManager.PeerRequestsPiece", request=request, peer=self
            )
    ...
    def _handle_keep_alive(self):
        try:
            ...
            logging.exception(
                "Error KeepALive, (need at least 4 bytes : {})".format(
    ...
    def get_messages(self):
        while len(self.read_buffer) > 4 and self.healthy:
            if (
                not self.has_handshaked and self._handle_handshake()
