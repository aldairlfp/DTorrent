...
class torrent_statistics:
    ...
    def update_download_rate(self, piece_index, piece_size):
        ...
        self.file_downloading_percentage = round(
            (len(self.downloaded) * 100) / self.total_pieces, 2
        )
        # time remaining for complete download
    ...
    def get_download_statistics(self):
        download_log = (
            "File downloaded : " + str(self.file_downloading_percentage) + " % "
        )
        download_log += (
            "(Average downloading rate : " + str(self.avg_download_rate) + " Kbps  "
        )
        download_log += (
            "Time remaining : " + str(self.expected_download_completion_time) + ")"
        )
        return download_log
