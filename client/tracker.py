...
from client.torrent_error import *
...
class tracker_data:
    ...
    def __init__(self, torrent):
        ...
        self.request_parameters = {
            "info_hash": urllib.parse.quote(torrent.torrent_metadata.info_hash.hex()),
...
class http_torrent_tracker(tracker_data):
    ...
    def request_torrent_information(self):
        ...
        try:
            ...
            bencoded_response = requests.get(
                self.tracker_url, self.request_parameters, timeout=5
            )
            ...
            raw_response_dict = json.loads(bencoded_response.content)
    ...
    def parse_http_tracker_response(self, raw_response_dict):
        ...
        if "peers" in raw_response_dict:
            ...
            # raw_peers_list = [
            #     raw_peers_data[i : 6 + i] for i in range(0, len(raw_peers_data), 6)
            # ]
            raw_peers_list = raw_peers_data
            ...
            self.peers_list += raw_peers_list[:]
    ...
    def __str__(self):
        ...
        # peer_data = "(" + str(self.peers_list[0][0]) + " : "
...
class udp_torrent_tracker(tracker_data):
    ...
    def build_connection_payload(self):
        req_buffer = struct.pack(
            "!q", self.connection_id
        )  # first 8 bytes : connection_id
        req_buffer += struct.pack("!i", self.action)  # next 4 bytes  : action
        req_buffer += struct.pack(
            "!i", self.transaction_id
        )  # next 4 bytes  : transaction_id
        return req_buffer
    ...
    def udp_connection_request(self, connection_payload):
        ...
        self.tracker_sock.sendto(
            connection_payload, (self.tracker_url, self.tracker_port)
        )
    ...
    def udp_announce_request(self, announce_payload):
        ...
        while trails < 8:
            ...
            try:
                self.tracker_sock.sendto(
                    announce_payload, (self.tracker_url, self.tracker_port)
                )
                ...
                error_log = (
                    self.tracker_url
                    + " failed announce request attempt "
                    + str(trails + 1)
                )
...
class torrent_tracker:
    ...
    def __str__(self):
        ...
        if unsuccessful_tracker_url:
            ...
            trackers_table.rows.append(
                [unsuccessful_log, "failed connection " + FAILURE]
            )
