from client.bencoder import bdecode
...
def parse_info(info):
    ...
    if b"files" in info:
        for file in info[b"files"]:
            ...
            file_dict["md5sum"] = (
                file[b"md5sum"].decode() if b"md5sum" in file else None
            )
...
def parse_torrent(torrent_data):
    ...
    torrent_dict["announce-list"] = [
        [a.decode() for a in announce] for announce in torrent_data[b"announce-list"]
    ]
