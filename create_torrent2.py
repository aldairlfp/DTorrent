...
class Config(object):
    ...
    def __init__(
        self, path: Optional[str] = None, advertise: Optional[bool] = True
    ) -> None:
...
def create_multi_file_info(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_FUTURES) as executor:
        ...
        for file in files:
            ...
            printv(
                "Processing file '%s'... "
                % clean_str_for_console(os.path.relpath(path, directory)),
                end="",
            )
...
def create_torrent(
    ...
    if invalid_nodes and not force:
        if _parser is not None:
            ...
            raise_error(
                "Some DHT bootstrap nodes are invalid, and force flag is not set."
            )
    ...
    excluded_regexps |= set(
        re.compile(regexp, re.IGNORECASE) for regexp in exclude_pattern_ci
    )
    ...
    if os.path.isfile(input_path):
        info = create_single_file_info(
            input_path, piece_length, include_md5, threads=threads
        )
    else:
            ...
            if os.path.isfile(output_path):
                if not force and os.path.exists(output_path):
                    if _parser is not None:
                        ...
                        raise_error(
                            "The specified path exists already, and force flag is not set."
                        )
        ...
        # The user specified a filename:
            ...
            if os.path.isfile(output):
                if not force and os.path.exists(output):
                    if _parser is not None:
                        if "yes" != input(
                            "'%s' does already exist. Overwrite? yes/no: " % output
                        ):
                            raise_error("Aborted.", _parser)
                    else:
                        raise_error(
                            "The specified file exists already, and force flag is not set."
                        )
